...
/**
 * Emblem resmi ww.cons, di-inline dari public/images/ww/ww_logo_official.svg.
 *
 * Di-inline (bukan <img src>) karena path-nya memakai fill="currentColor" —
 * hanya mewarisi warna teks kalau berada di DOM yang sama. Itu juga yang
 * memungkinkan animasi wipe di preloader tanpa file video sama sekali.
 *
 * Sebelumnya komponen ini memakai PNG 512x512 dan menyimpan prop `videoSrc`,
 * `gold`, serta varian `preloader`/`minimal` yang tidak pernah dipakai.
 */
export function WwLogoMark({ className = '' }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 378 512"
      fill="none"
...
export default function ModernWwLogo({
  ...
  const s = SIZE[size];
  const mark = <WwLogoMark className={`${s.box} shrink-0 text-white`} />;
